...
| 净值 | `/sim/equity?days=90` | GET | 账户净值走势（资产曲线，按日快照序列，支持 30/90/180 天） |
| 行情 | `/market/home` | GET | 主页热门标的事实时行情（推荐池，后台可维护） |
| | `/market/quote?symbol=sh600519` | GET | 单标的实时行情 |
| | `/market/kline?symbol=sh600519&count=60` | GET | 单标的 K 线（收盘价序列，详情页迷你走势图用） |
| | `/market/search?q=茅台` | GET | 行情名称/代码搜索（腾讯 smartbox，命中自动写入 searched_symbols，场外基金顺带补净值） |
| | `/market/overview` | GET | 全市场涨跌概览（上涨/下跌/平盘家数，优先当日落库数据，否则腾讯排行实时统计） |
...
- `sim_equity_snapshots` — 每日净值快照（总资产/现金/市值/盈亏，资产走势曲线数据源）
...
三套页面（`/home` 行情中心、`/sim` 模拟交易、`/admin` 后台）共用一套设计系统，纯原生 HTML/CSS/JS（由 FastAPI 直接托管 `static/`），**不依赖任何 CDN / 第三方图表库**，可离线、自包含运行。

- **浅色为主 + 深色切换**：`<html data-theme>` 切换主题，选择持久化于 `localStorage(sim_theme)`；切换时派发 `themechange` 事件，图表自动重绘。
- **统一应用壳层**：`App.mount(active)` 注入顶栏（品牌 / 导航 / 主题切换 / 登录态），各页面只需关注自身内容。
- **行情中心 `/home`**：市场概览（涨跌家数 + 涨跌分布条）、分类标签（推荐/指数/股票/ETF/基金）、行情搜索联想下拉、行情卡片（颜色标记涨跌）、15 秒自动刷新开关；点击卡片弹出详情（报价 + 近 60 日迷你走势图）或场外基金净值详情。
- **模拟交易 `/sim`**：账户指标网格、资产净值走势曲线（30/90/180 天分段）、持仓配置环形图（现金/股票/基金）、快速交易面板（实时价预览 + 费用估算，股票/基金场内·场外逻辑分流）、持仓与流水表格、账户重置。支持从主页带 `?code=&mt=` 深链预填。
- **后台 `/admin`**：登录/注册、默认初始资金保存、补充默认数据、同步基金净值、热门标的增删改。
- **零依赖 SVG 图表**：`static/charts.js` 提供折线/面积图（带 hover 提示、响应式重绘）与环形图，颜色读取自 CSS 变量，跟随主题变化。
- **配色约定**：沿用 A 股习惯 —— 涨=红、跌=绿（由 `.up`/`.down` 类驱动）。

### 资产净值曲线

`/sim/equity` 的数据来自每日快照表 `sim_equity_snapshots`：调度器在**每个交易日 15:30**（`equity_snapshot` 任务）为全部模拟账户写入/更新当日净值（同一天幂等）。曲线即该序列的升序走势，历史越久越平滑。
...
### v3.3 - 全市场概览 + 搜索入库 + 交易时间规则

**市场数据**
- ✅ 行情中心市场概览改为**全市场 A 股涨跌统计**（上涨/下跌/平盘家数）：优先当日落库全量数据，否则实时拉腾讯行情排行接口统计（5 分钟缓存）
- ✅ 每日 15:30 全市场更新增加**腾讯排行全量降级源**（东财/akshare 不可达时自动降级，覆盖约 5500 只股票）
- ✅ 修复指数行情被误判为深市的问题（上证指数曾显示成平安银行股价）

**搜索**
- ✅ 任意用户搜索股票/基金后**自动入库** `searched_symbols`（去重累计次数），场外基金搜索时顺带补抓净值
- ✅ 修复搜索解析（腾讯 smartbox v2 返回格式 `v_hint="sh~600519~贵州茅台~gzmt~GP-A"`，支持多结果与基金 `jj` 前缀）
- ✅ 基金净值同步任务覆盖搜索记录中的场外基金

**交易规则**
- ✅ 股票买卖**仅允许交易时段**（工作日 9:30-11:30 / 13:00-15:00），非交易时段后端拒绝并提示
- ✅ 基金任意时间可申赎；场外基金工作日 15:00 后下单按**下一工作日（T+1）净值确认**，流水展示确认日
- ✅ 修复交易时间显示时区（库内 UTC 时间前端按北京时间展示）
...
**前端**
- ✅ 三页统一设计系统（浅色为主 + 深色一键切换，主题持久化）
- ✅ 行情中心：涨跌分布、搜索联想、行情卡片、详情迷你走势图、自动刷新
- ✅ 模拟交易：账户指标、资产净值曲线（30/90/180 天）、持仓配置环形图、实时价预览快速交易
- ✅ 后台：默认参数与热门标的维护界面美化
- ✅ 零依赖 SVG 图表（`static/charts.js`），不依赖任何 CDN，离线可用

**后端 / API**
- ✅ 新增 `GET /sim/equity` —— 账户净值走势（资产曲线）
- ✅ 新增 `GET /market/kline` —— 单标的 K 线（收盘价序列）
- ✅ 新增 `GET /market/search` —— 行情名称/代码搜索（腾讯 smartbox）
- ✅ 新增 `sim_equity_snapshots` 表 + 每日 15:30 净值快照定时任务
- ✅ `StockDataClient.search()` 多源行情搜索能力
